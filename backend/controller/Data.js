...
export const getAllAverageData = async (req, res, next) => {
  try {
    // Mendapatkan semua data rata-rata dari database
    const averageData = await Average.findAll();

    // Mengambil hanya nilai CO dan createdAt dari setiap data
    const formattedData = averageData.map(({ CO, createdAt }) => ({
      CO,
      createdAt,
    }));

    // Mengirim data rata-rata sebagai respons API
...
export const donwloadExcel = async (req, res, next) => {
  try {
    ...
    worksheet.columns = [
      ...
      { header: "CreatedAt", key: "createdAt", width: 20 }, // Kolom createdAt
